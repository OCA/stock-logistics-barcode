...
class WizStockBarcodesMapping(models.Model):
    ...
    model_label_id = fields.Many2one('ir.model', string="Model of the label" , required=True)
    item_ids = fields.One2many('wiz.stock.barcodes.mapping.item', 'mapping_id', required=True)
    ...
    def prepare_label(self, model, items):
        json = "{"
        for item in items:
            json += "'%s': '%s'," % (item.key, item.value)
        json += "}"
        return {'name': 'Label for '+ model.name,
                'sequence': 32,
                'model_id': model.id,
                'component_ids': [(0,0,{'name': 'QR json',
