...
from .exceptions import InvalidGS1Barcode


class GS1Barcode(orm.Model):
    """GS1-128/GS1-Datamatrix barcode decoder API and configuration"""
    _name = "gs1_barcode"
    _description = __doc__
    _columns = {
        'ai': fields.char(
            'Application Identifer', size=14,
            help='The standard Application Identifier (AI)', required=1,
            select=1),
        'name': fields.char(
            'Description', size=64, required=True, select=1, translate=True),
        'length_fixed': fields.boolean(
            'Fixed-length Data',
            help="Indicates whether the length of the data for this "
                 "Application Identifier is fixed or not."),
        ...
        'length_min': fields.integer(
            'Minimum Data Length',
            help="Minimum length of the data for this Application "
                 "Identifier."),
        'decimal': fields.boolean(
            'Decimal Indicator',
            help="Indicates whether a digit is expected before the data for "
                 "this Application Identifier to indicate the position of "
                 "the decimal point."),
        'type': fields.selection(
            [('string', 'Any character string'),
             ('numeric', 'Numeric value'),
             ('date', 'Date')], 'Data Type', required=1),
    }
    _defaults = {
        'length_fixed': True,
        'length_max': 30,
    ...
    def decode(self, cr, uid, barcode_string, context=None):
        ...
        sep = user.gs1_barcode_separator or '\x1D'

        if not barcode_string.startswith(prefix):
            raise InvalidGS1Barcode(
                _('Error decoding barcode'),
        ...
        VARIABLE_LENGTH = r'(?P<value>[^' + sep + r']{%d,%d}' + sep + r'?)'
        ...
        # Make a dictionary of compiled regexps to decode the string
        ai_regexps = {}
        value_regexps = {}
        types = {}
        ...
        while position < len(barcode_string):
            ...
            for (ai, regexp) in ai_regexps.items():
                ...
                if match:
                    ...
                    results[ai] = groups['value'].replace(sep, '')
                    if types[ai] == 'numeric':
                ...
                # No other valid AI found in the rest of the code: give up
                raise InvalidGS1Barcode(
                    _('Error decoding barcode'),
